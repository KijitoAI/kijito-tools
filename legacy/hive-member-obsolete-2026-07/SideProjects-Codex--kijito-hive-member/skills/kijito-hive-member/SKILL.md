...
2. Run the exact hook-emitted `pointer-snapshot.mjs` command for the configured
...
- At or above 70%, stop ordinary work and run `$kijito-qa-memory`
  immediately. Do not wait for automatic compaction; the ceiling is recovery,
  not a planned handoff boundary.
- Before compaction, use `$kijito-qa-memory` to run bounded curation, publish
...
- Publish only with the hook-emitted `pointer-publish.mjs` command. It claims
...
5. Refresh the living pointer only through the canonical publisher.
