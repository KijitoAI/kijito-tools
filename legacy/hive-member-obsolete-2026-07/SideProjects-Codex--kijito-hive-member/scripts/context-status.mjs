...
export function contextReminder(status) {
  ...
  let boundary;
  if (status.usedPercent >= 70) {
    boundary = "At or above the 70% mandatory handoff boundary: stop ordinary work and run $kijito-qa-memory now; do not wait for automatic compaction.";
  } else if (status.usedPercent >= 60) {
    boundary = "At or above the 60% planning boundary: prepare a QA-verified handoff at the next clean boundary, before 70% usage.";
  } else {
    boundary = "Below the 60% planning boundary: continue; do not recycle based on felt context.";
  }
