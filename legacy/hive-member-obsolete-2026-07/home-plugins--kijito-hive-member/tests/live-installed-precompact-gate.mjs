...
if (!Number.isSafeInteger(pointerId)
  ...
  throw new Error(
    "Set KIJITO_POINTER_ID and KIJITO_POINTER_DIGEST to the pointer revision verified by two clean cold boots.",
  );
}
...
try {
  ...
  const blockedPreCompactPromise = client.waitForNotification(
    (message) => (
  ...
  const blockedPreCompact = await blockedPreCompactPromise;
  ...
  const hookRuns = notifications
    ...
    .filter(Boolean);
  assert.equal(
    blockedPreCompact.params?.run?.status,
    "stopped",
    "PreCompact must report the documented stopped state",
  );
  assert.equal(compacted, false, "compaction must not complete without QA");
  assert.equal(
    compactionCompleted,
    false,
    "no contextCompaction item may complete without QA",
  );
  assert.equal(
    hookRuns.some((run) => run.eventName === "postCompact"),
    false,
    "PostCompact must not run after PreCompact stops the attempt",
  );
  ...
  assert.equal(
    ...
    0,
    "the blocked attempt must not persist a contextCompaction item",
  );
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    blockedWithoutPass: {
      ...
      preCompactRun: blockedPreCompact.params?.run,
      hookRuns,
    },
    allowedWithPass: {
      pointerId,
