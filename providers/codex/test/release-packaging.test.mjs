...
test("an internally consistent alternate source tree cannot impersonate the reviewed commit", () => {
  ...
  try {
    ...
    // Resolution note: the union refresh tool also hashes ../../.github/workflows/test.yml,
    // so the impersonating tree needs one for its refresh to complete (still not a reviewed file).
    const altWorkflows = path.join(f.root, ".github", "workflows");
    fs.mkdirSync(altWorkflows, { recursive: true, mode: 0o700 });
    fs.copyFileSync(path.join(packageRoot, "..", "..", ".github", "workflows", "test.yml"),
      path.join(altWorkflows, "test.yml"));
...
test("smoke command fences armed evidence to bytes written after its own start", () => {
  ...
  // Resolution note (merge of PR#6 + main): the offset is now taken from the fstat of the just-
  // opened private log fd (stronger than the parent's existsSync/statSync form this pin replaced).
  assert.match(cli, /const logOffset = logStat\.size/);
  ...
  for (const relative of [
    ...
    try {
      ...
      try {
        ...
        // Resolution note: PR#6's verifySourceCommit refuses a non-repository staged tree before
        // the hash gate can speak; either refusal proves the tampered source cannot install.
        assert.match(result.stderr, /differs? from gated hash|source commit verification failed|source artifact differs from reviewed commit/, relative);
