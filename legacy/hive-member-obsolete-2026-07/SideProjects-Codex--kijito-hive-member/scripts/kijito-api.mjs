...
export async function claimMessageLease({
  messageId,
  persona,
  leaseSeconds = 60,
  tokenFile,
  ...
  if (!validMessageId(messageId)
    ...
    || leaseSeconds < 10
    || leaseSeconds > 300) {
    throw Object.assign(new Error("message lease claim is invalid"), {
      code: "message_lease_claim_invalid",
    });
  }
  ...
  if (typeof data?.claimed !== "boolean" || data.message_id !== messageId) {
    throw Object.assign(new Error("message lease claim response is invalid"), {
      code: "message_lease_claim_response_invalid",
    });
  }
  if (!data.claimed) {
    ...
    if (!advisory
      ...
      || typeof advisory.claimed_by !== "string"
      || typeof advisory.lease_expired !== "boolean") {
      throw Object.assign(new Error("message lease refusal advisory is invalid"), {
        code: "message_lease_claim_response_invalid",
      });
    }
  }
...
export async function releaseMessageLease({
  messageId,
  persona,
  tokenFile,
  timeoutMs = 5000,
  ...
  if (!validMessageId(messageId) || !validPersona(persona)) {
    throw Object.assign(new Error("message lease release is invalid"), {
      code: "message_lease_release_invalid",
    });
  }
  const token = readToken(tokenFile);
  if (!token) {
  ...
  const data = await requestJson({
    requestPath: "/api/release",
    token,
    method: "POST",
    body: { message_id: messageId, persona },
    timeoutMs,
    requestImpl,
  });
  if (typeof data?.released !== "boolean" || data.message_id !== messageId) {
    throw Object.assign(new Error("message lease release response is invalid"), {
      code: "message_lease_release_response_invalid",
    });
  }
  return data;
}
