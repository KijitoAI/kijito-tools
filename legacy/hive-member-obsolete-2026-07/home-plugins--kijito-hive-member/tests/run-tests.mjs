...
import { sendMessage } from "../scripts/kijito-api.mjs";
import {
  planMemoryActions,
  shouldDream,
  stopChecklist,
...

{
  ...
  try {
    ...
    assert.match(blocked.systemMessage, /\$kijito-qa-memory/);
    assert.match(blocked.systemMessage, /REPLACE_WITH_POINTER_ID/);
    assert.match(blocked.systemMessage, /pointer-digest\.mjs/);
    assert.match(blocked.systemMessage, /REPLACE_WITH_POINTER_DIGEST/);
    assert.match(blocked.systemMessage, /otherwise restart both boots/i);
    recordQaPass({
      dataDir: qaDir,
      sessionId: "session-one",
      transcriptPath: transcript,
...

{
  ...
  assert.match(
    firstPostCompactJson.hookSpecificOutput.additionalContext,
  ...
  assert.equal(malformedPreCompactJson.continue, false);
  assert.match(malformedPreCompactJson.stopReason, /failed closed/);
  assert.match(malformedPreCompactJson.systemMessage, /do not bypass memory QA/);
  ...
  assert.equal(wrapperSuccessJson.continue, false);
  assert.match(wrapperSuccessJson.stopReason, /memory QA must pass/);
  assert.match(wrapperSuccessJson.systemMessage, /no fresh, session-bound/);
  ...
  assert.equal(wrapperFailureJson.continue, false);
  assert.match(wrapperFailureJson.stopReason, /runtime failed/);
  ...
  assert.equal(wrapperNodeFailureJson.continue, false);
  assert.match(wrapperNodeFailureJson.systemMessage, /runtime failed safely/);
  ...
  assert.equal(wrapperTimeoutJson.continue, false);
  assert.match(wrapperTimeoutJson.stopReason, /runtime failed/);
  assert.equal(
  ...
  assert.equal(oversizedOutputJson.continue, false);
  assert.match(oversizedOutputJson.stopReason, /runtime failed/);
  ...
  assert.equal(JSON.parse(mismatchedPreCompact.stdout).continue, false);
...

{
  ...
  const captureCommandsAt = patternIndex(
    qaSkill,
    /Before spawning either boot, obtain the exact/,
  );
  const spawnBootAt = patternIndex(
    qaSkill,
    /Spawn a fresh context-free agent/,
  );
  const finalDigestAt = patternIndex(
    qaSkill,
    /immediately before recording/,
  );
  const finalRecordAt = patternIndex(
    qaSkill,
    /Run the record command as the final\s+memory-QA action/,
  );
  const nativeCompactAt = patternIndex(
    qaSkill,
    /Immediately request native Codex compaction/,
  );
  const beforeReadAt = patternIndex(
    qaSkill,
    /immediately before reading Kijito/,
  );
  const connectAt = patternIndex(
    qaSkill,
    /Then connect to Kijito as persona/,
  );
  const afterReadAt = patternIndex(
    qaSkill,
    /Immediately after reading and evaluating/,
  );
  const recallIdAt = patternIndex(
    qaSkill,
    /one unambiguous top current-state result whose ID equals the pointer/,
  );
  const dreamAt = patternIndex(
    qaSkill,
    /run `kijito_dream` now/,
  );
  const pointerPreloadAt = patternIndex(
    qaSkill,
    /## 3\. Preload the pointer/,
  );
  assert.ok(captureCommandsAt >= 0 && captureCommandsAt < spawnBootAt);
  assert.ok(
    beforeReadAt > spawnBootAt
      && beforeReadAt < connectAt
      && connectAt < recallIdAt
      && recallIdAt < afterReadAt,
  );
  assert.match(qaSkill, /only\s+>\s*non-Kijito actions/);
  assert.match(
    qaSkill,
  ...
  assert.match(qaSkill, /recall-selected ID and digest as one verified pair/);
  assert.ok(dreamAt < pointerPreloadAt && pointerPreloadAt < spawnBootAt);
  assert.match(qaSkill, /single-writer\s+pointer lease/);
  assert.doesNotMatch(
    qaSkill,
    /explicitly linked load-bearing memory or edge/,
  );
  assert.match(qaSkill, /another seat to edit it\s+during the final-check-to-record window/);
  assert.match(qaSkill, /cryptographically binds the exact pointer content only/);
  assert.match(
    qaSkill,
  ...
  assert.match(qaSkill, /retired audit history, regardless of importance/);
  assert.match(qaSkill, /Importance may corroborate\s+> retirement but must never gate it/);
  assert.match(qaSkill, /SUPERSEDED BY <newer-id> — audit history, NOT current/);
  assert.match(qaSkill, /bounded, fenced preview/);
  assert.match(
    qaSkill,
    /unavoidable presence in the pointer response does not fail the boot/,
  );
  assert.match(qaSkill, /Never\s+> call `kijito_get` separately on a retired predecessor/);
  assert.match(qaSkill, /full body, fail the boot/);
  assert.doesNotMatch(
    qaSkill,
  ...
  assert.ok(finalDigestAt > spawnBootAt && finalDigestAt < finalRecordAt);
  assert.ok(nativeCompactAt > finalRecordAt);
  assert.match(qaSkill, /`kijito\.compaction\.ready` signal/);
  assert.match(qaSkill, /`thread\/compact\/start`/);
  assert.match(qaSkill, /Do not call `thread\/resume`/);
  assert.match(qaSkill, /do not.*substitute\s+`\/clear`/is);
  assert.match(
    qaSkill,
    /missing,\s+malformed, or different digest is a failed boot/,
  );
  ...
  assert.match(startSkill, /exactly once for each successful compaction/);
  assert.match(startSkill, /`SessionStart\(compact\)` is a no-op/);
  assert.match(startSkill, /duplicate `PostCompact`.*also a no-op/s);
  assert.match(startSkill, /persistent autonomous-goal/);
  assert.match(
    startSkill,
    /predecessor end of a `version_of`, `derived:version_of`, or\s+`version_history` edge from a newer memory/,
  );
  assert.match(startSkill, /retired audit history,\s+regardless of importance/);
  ...
  assert.match(startSkill, /Never call\s+`kijito_get` separately on a retired predecessor/);
  ...
  assert.match(parity, /Preemptive native compaction at the measured 60% boundary/);
  ...
  assert.match(readme, /`PostCompact` alone atomically claims that ticket/);
...
console.log(JSON.stringify({
  ...
  checks: [
    ...
    "hook JSON protocol and QA-gated compaction lifecycle",
    "memory engagement",
